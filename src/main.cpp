...
const string strMessageMagic = "MicroCash Signed Message:\n";
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in MicroCash
    bool fStrictPayToScriptHash = true; // Always active in MicroCash
...
bool CBlock::AcceptBlock()
{
    ...
    if (nVersion != (BLOCK_VERSION_SHA256D | BLOCK_VERSION_DEFAULT) &&
        nVersion != (BLOCK_VERSION_SCRYPT  | BLOCK_VERSION_DEFAULT) &&
        nVersion != (BLOCK_VERSION_GROESTL | BLOCK_VERSION_DEFAULT) &&
	nVersion != (BLOCK_VERSION_X11     | BLOCK_VERSION_DEFAULT) &&
	nVersion != (BLOCK_VERSION_X13	   | BLOCK_VERSION_DEFAULT) &&
	nVersion != (BLOCK_VERSION_X15	   | BLOCK_VERSION_DEFAULT) &&
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "MicroCash", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
CBlock* CreateNewBlock(CWallet* pwallet, bool fProofOfStake, int algo)
{
	...
	case ALGO_X13:
	    ...
	    break;
	case ALGO_X15:
	    pblock->nVersion |= BLOCK_VERSION_X15;
	    break;
	case ALGO_QUARK:
...
//    if (algo == ALGO_BLAKE) { printf("Algo is Blake \n"); }
    ...
    if (hashPoW > hashTarget && pblock->IsProofOfWork())
        return error("MicroCashMiner : proof-of-work not meeting target");

    uint256 hashBlock = pblock->GetHash();

    //// debug print
    printf("MicroCashMiner:\n");
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("MicroCashMiner : generated block is stale");
        ...
        if (!ProcessBlock(NULL, pblock))
            return error("MicroCashMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ThreadMicroCashMiner(void* parg);

static bool fGenerateBitcoins = false;
static bool fLimitProcessors = false;
static int nLimitProcessors = -1;

void MicroCashMiner(CWallet *pwallet, bool fProofOfStake)
{
    printf("CPUMiner started for proof-of-%s\n", fProofOfStake? "stake" : "work");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);

    // Make this thread recognisable as the mining thread
    RenameThread("microcash-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake)
    {
        ...
        printf("Running MicroCashMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadMicroCashMiner(void* parg)
{
    ...
    try
    {
        ...
        MicroCashMiner(pwallet, false);
        vnThreadsRunning[THREAD_MINER]--;
    }
    catch (std::exception& e) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(&e, "ThreadMicroCashMiner()");
    } catch (...) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(NULL, "ThreadMicroCashMiner()");
    }
    nHPSTimerStart = 0;
    if (vnThreadsRunning[THREAD_MINER] == 0)
        dHashesPerSec = 0;
    printf("ThreadMicroCashMiner exiting, %d threads remaining\n", vnThreadsRunning[THREAD_MINER]);
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    if (fGenerate)
    {
        ...
        printf("Starting %d MicroCashMiner threads\n", nAddThreads);
        for (int i = 0; i < nAddThreads; i++)
        {
            if (!NewThread(ThreadMicroCashMiner, pwallet))
                printf("Error: NewThread(ThreadMicroCashMiner) failed\n");
            Sleep(10);
        }
    }
}
